  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mt-10 flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900 sm:text-3xl">Ingestion</h1>
          <p className="mt-1 text-sm text-neutral-500">Data engine diagnostics and provider status.</p>
        </div>
      ...
      {overview && (
        <>
          ...
          <section className="mt-8 rounded-2xl border border-neutral-200 bg-white p-5">
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-neutral-500">Providers</h2>
            <div className="flex flex-col gap-3">
              {overview.providers.map((provider) => (
                <div key={provider.id} className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <p className="max-w-xl text-xs text-neutral-500">{provider.message}</p>
                </div>
              ))}
            </div>
            <p className="mt-3 text-xs text-neutral-400">
              Active provider: <span className="font-semibold text-neutral-700">{overview.activeProvider}</span>. The LIVE badge only appears when a data stream is genuinely live.
            </p>
          </section>

          {overview.lastRun && (
            <section className="mt-6 rounded-2xl border border-neutral-200 bg-white p-5">
              <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-neutral-500">Last ingestion run</h2>
              ...
              <p className="mt-3 text-xs text-neutral-400">
                Status: <span className="font-semibold text-neutral-700">{overview.lastRun.status}</span>
          ...
          {runMessage && (
            <p className={`mt-4 text-sm ${runMessage.includes('failed') ? 'text-red-600' : 'text-emerald-600'}`}>{runMessage}</p>
          )}

          {ingestion && ingestion.runs.length > 0 && (
            <section className="mt-6 rounded-2xl border border-neutral-200 bg-white p-5">
              <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-neutral-500">Recent runs</h2>
                  ...
                  <tbody>
                    {ingestion.runs.map((run) => (
                      <tr key={run.id} className="border-b border-neutral-50">
                        ...
                        <td className="py-2 text-neutral-500">{run.startedAt}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <ul className="flex flex-col gap-1.5 text-sm">
                {ingestion.errors.slice(0, 15).map((e) => (
                  <li key={e.id} className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-neutral-500">{e.externalId ?? '—'}</span>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-4">
      ...
      <p className="mt-0.5 text-xs font-medium uppercase tracking-wider text-neutral-400">{label}</p>
    </div>
  )
}
